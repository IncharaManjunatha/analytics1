...
library(VIM)
(v1 = c(1,2,NA,NA,5)) # create vector with missing values
is.na(v1)
v1=NULL
...
mean(v1, na.rm=T)  # remove missing valus and calc
...
library(VIM)
data(sleep, package='VIM')    # sleep is dataset with missing values
...
mean(complete.cases(sleep))  #42/62  68% rows are complete
...
colSums(is.na(sleep))  #missing(NA) values in each column
