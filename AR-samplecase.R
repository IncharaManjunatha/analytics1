...
summary(tdata)
itemlist
image(tdata)  # displays transactions in graphical form

#Analysis
freqitems = eclat(tdata) #default support=.1
#freqitems = eclat(tdata, parameter = list(minlen=1, supp=.01, maxlen=4 ))

freqitems
inspect(freqitems)

support(items(freqitems[1]), transactions=tdata)
support(items(freqitems[1:5]), transactions=tdata)
inspect(freqitems[1])
...
write.csv(as.data.frame(inspect(freqitems)),'freqitems1.csv') # write into file


#Construct the Rules
rules = apriori(tdata, parameter = list(supp = 0.1, conf = 0.5, minlen=2))
...
# for each rule, suppm conf lift and count are printed
quality(rules_c) 
...
sum(is.redundant(rulesNR))  #ok now # count the redundant rules
inspect(rulesNR)
