...
sum(df$X); sum(df$Y)
sd(df$X) ; var(df$Y)
cov(df$X,df$Y); cov(df$Y,df$X)      # tells the relation or direction (directly or indirectly related)
cor(df$X,df$Y) ; cor(df$Y,df$X)     # strength or magnitude of relation or direction 
#cor.test(df$X,df$Y)
...
fit1
summary(fit1)  # detailed analysis of lm
...
plot(fit1, which=1) #dianostic plot 
plot(fit1)          #4 plots
par(mfrow=c(2,2)) # display four at once
plot(fit1)
par(mfrow=c(1,1))
...
# 2 types: auto or 
# null hypothesis is accepted since p-value>0.05
...
#Normality of residuals means normal dist of residuals
plot(fit1, which=2) # normal q 
...
summary(fit)
plot(fit)
predict(fit.newdata=data.frame(df$X))
