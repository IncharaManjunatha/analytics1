...
# logistic regression is binary classification where the output is in probability
# Values thrown will be between 0 and 1

## importing .csv file from internet domain
...
inputData
summary(inputData)
sapply(inputData, sd) # sd for each column
str(inputData)
...
# predicted>0.5 --- assign 1 
# but not always true
# So, we are going to predict cutoff value
#cutoff value
library(InformationValue)
(optCutOff <- optimalCutoff(data$admit, prob)[1] ) 
...
(accuracy = (83+109)/ (sum(83+109+18+190))) #.48

## view data frame
library(dplyr)
sample_n(data,size=1)

# rank was converted to factor in the beginning
